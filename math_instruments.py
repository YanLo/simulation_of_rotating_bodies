...
def rungekut(func, t_0, t_1, x_0):
    ...
    for i in range(num_of_it):
        k_1 = func(t_cur, x_cur)
        k_2 = func(t_cur + step/2, np.add(x_cur, np.dot(k_1, step/2)))
        k_3 = func(t_cur + step/2, np.add(x_cur, np.dot(k_2, step/2)))
        k_4 = func(t_cur + step, np.add(x_cur, np.dot(k_3, step)))
